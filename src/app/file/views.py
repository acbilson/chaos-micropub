...
from app.operators import compose_header, null_or_empty, git_pull, git_commit
...
def read():
    ...
    return jsonify(
        success=True, message="", result=dict(filepath=file_path, content=content)
    )
...
def update():
    ...
    if null_or_empty(content):
        return jsonify(success=False, message=f"content was missing. {content}")

    if null_or_empty(options):
        return jsonify(success=False, message=f"options were missing. {options}")
    ...
    if git_message is not None:
        return jsonify(
            success=False,
            message=git_message,
            result=dict(filepath=file_path, content=content),
        )

    return jsonify(
        success=True, message="", result=dict(options=options, content=content)
    )
...
def create():
    ...
    if null_or_empty(content):
        return jsonify(success=False, message=f"content was missing.")

    if null_or_empty(options):
        return jsonify(success=False, message=f"options were missing.")
